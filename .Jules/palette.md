...
## 2026-09-04 - [Apply focus-visible styles to all text links uniformly]
**Learning:** Found that basic text links (like "Home" at the bottom of pages, or standard navigation links) often lack the global `focus-visible` classes, leaving them completely inaccessible to keyboard users navigating via Tab.
**Action:** Always verify that every `Link` or `<a>` element acting as a navigation element includes standard `focus-visible:outline-none focus-visible:ring-2 rounded` styles to ensure clear visual focus indicators.

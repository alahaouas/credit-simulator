...
## 2026-09-08 - [Audit all links for focus-visible styles]
**Learning:** While primary call-to-action buttons are often styled correctly with keyboard focus indicators, secondary elements like header/footer text links and alternate state buttons (e.g., "History" or "Sign In" vs the primary "Run") frequently miss these standard accessibility classes.
**Action:** When adding `focus-visible` states to a component, comprehensively audit all interactive elements (links, secondary buttons) within the file to ensure consistent keyboard accessibility coverage.

...
## 2026-08-26 - A `min-width` sized for English is not a fix for layout shift
**Learning:** Eight Palette PRs (#212–#221) proposed the same `aria-live="polite"` + `min-w-[90px]` pair for the two copy buttons. The live region is right — a text node swapped in place is announced by nothing — but the fixed min-width only holds in English: `history.share_copy` is "Copy link" (~87px) in EN and "Copier le lien" (~114px) in FR, so the FR button still collapses to 90px on toggle. A magic pixel number silently re-breaks on every new locale or copy edit. Sweeping the tree also showed the dead-end empty state was one page only: `history` already pairs its empty text with a CTA, and `alerts`/`settings` keep their creation form on the same screen, so `compare` — whose only action lives on another route — was the sole genuine dead end.
**Action:** Reserve space from the strings themselves, not from a measured pixel: stack every label variant in one CSS grid cell (`grid` + `col-start-1 row-start-1`, the inactive ones `invisible aria-hidden`) so the control always sizes to the longest translation. Keep `aria-live="polite"` on the control for the announcement. Before adding an empty-state CTA, check whether the action is already reachable on the same screen — only a state whose next step lives on another route is a dead end.

## 2026-09-10 - Copy the focus ring the repo already uses; do not invent a fourth one
**Learning:** Six Palette PRs (#225, #229, #230, #231, #233, #234) proposed the same focus-visible sweep and each picked a different ring. `dark:focus-visible:ring-gray-600` (#230, #231) is darker than the `gray-900` page it sits on; `ring-offset-2` without `dark:focus-visible:ring-offset-gray-900` paints a white halo in dark mode; `rounded` appended to an element already `rounded-lg` is dead weight. The tree already answers this: `grep -rhoE 'focus-visible:outline-none[^"]*'` returns three strings covering 45 of 48 uses — solid CTA (ring-black / dark ring-white / offset), bordered control (ring-gray-400 + offset + dark offset), inline text link (ring-gray-400 + rounded, no offset). No PR covered all 14 unstyled elements either, and every pair conflicted on the same lines.
**Action:** Before styling a state, tally the existing class strings with `grep -rhoE` and reuse the majority variant verbatim; a new colour or a missing dark-mode offset is a regression, not a choice. Sweep the whole tree in one pass — a per-page PR guarantees a conflict with the next per-page PR on the same file.

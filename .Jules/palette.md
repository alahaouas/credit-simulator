...
## 2026-09-05 - [Apply focus-visible to navigation links consistently]
**Learning:** Found that basic text navigation `<Link>` and `<a>` elements (like "Home" or "Preferences") lacked `focus-visible` styles, rendering them completely invisible to keyboard navigation.
**Action:** When adding or modifying Next.js `<Link>` or `<a>` elements, ensure they include Tailwind's `focus-visible` utility classes (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-400 dark:focus-visible:ring-gray-600 rounded`) for keyboard accessibility. Add `rounded` constraints if the component lacks a border radius so the focus ring aligns correctly.

...
## 2026-09-10 - [Add missing global state toggles on auth/standalone pages]
**Learning:** Global state controls (like language or theme toggles) are often inadvertently omitted from standalone or layout-isolated pages (like authentication flows), leading to a disjointed user experience where users are trapped in default states until they log in.
**Action:** When adding or auditing global controls, explicitly check standalone routes outside the primary application layout (`layout.tsx`) to ensure they receive the same components (e.g., `LocaleToggle` and `DarkModeToggle`) styled consistently.

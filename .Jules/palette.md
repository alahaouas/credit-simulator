...
## 2026-09-02 - [Avoid dead-end success states]
**Learning:** Found a literal dead-end UI in the authentication flow. After successfully requesting a magic link, the page simply displayed a text message ("Check your inbox...") with no navigation options, forcing users to use browser navigation or edit the URL if they wanted to return to the application while waiting.
**Action:** When designing or updating success states (like form submissions or auth requests), always include a clear Call-To-Action (CTA) such as a "Return Home" link (styled with proper `focus-visible` states) to ensure the user flow remains continuous and accessible without relying on browser chrome.

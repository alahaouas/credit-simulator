...
## 2026-08-16 - Add clear CTAs to dead-end empty states
**Learning:** Found a dead-end empty state in the compare page where the user was just told there was no data with no explicit button to take action. Replacing dead-end text with helpful visual Call-To-Action (CTA) buttons improves navigation flows and reduces user friction. We also identified that text loading states inside content panels create unpolished experiences and must be replaced with proper visual SVG spinners equipped with `aria-live` regions.
**Action:** When presenting empty states in frontend pages, always include a helpful call-to-action button directing the user exactly where to go next (e.g. back to a list view), and ensure all inline content panel loading states use animated SVG spinners with `aria-label` instead of plain text.

## 2026-08-16 - Avoid hardcoded pixel widths for localized transient states
**Learning:** Using hardcoded pixel widths (like `min-w-[90px]`) to prevent layout shifts when a button's text changes temporarily (e.g., from 'Copy' to 'Copied!') fails when the application is localized. For example, "Copier le lien" in French is much wider, causing the button to still collapse or shift. A better, more robust approach is to stack all possible label variants in a single CSS grid cell, allowing the browser to size the control to the longest translation without requiring manual pixel constants.
**Action:** When preventing layout shifts for transient text states in buttons, avoid using fixed `min-w` pixel values. Instead, use CSS grid techniques (like stacking variants in the same cell) to ensure the button accommodates the longest string across all supported locales.

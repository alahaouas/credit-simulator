...
## 2026-09-01 - [Add role="status" to non-button visual loading spinners]
**Learning:** While `aria-live="polite"` and `aria-busy="true"` are helpful for screen readers, missing `role="status"` on non-button loading spinner containers (like panel/form overlays) can prevent some assistive technologies from correctly prioritizing and announcing the state change.
**Action:** When adding or auditing SVG loading spinners outside of standard `<button>` elements, always wrap them in a container that explicitly includes `role="status"` alongside `aria-live="polite"`, `aria-busy="true"`, and a dynamic `aria-label`.

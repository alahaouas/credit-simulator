...
## 2026-08-28 - [Add focus-visible pattern to interactive navigation Links]
**Learning:** React Router / Next.js `<Link>` components that render as interactive navigation buttons or secondary actions are often styled like buttons (e.g. rounded boxes with borders) but miss the global `focus-visible` classes during development. Keyboard users navigating through pages (like Home, Settings, or history lists) will lose track of focus without them.
**Action:** When adding or auditing `<Link>` elements, particularly those styled as primary or secondary actions, ensure they include `focus-visible:outline-none focus-visible:ring-2` (along with `rounded` constraints if the outline needs structure) for consistent keyboard accessibility.

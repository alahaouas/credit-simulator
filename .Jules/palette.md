...
## 2026-09-10 - [Add focus-visible pattern to landing page navigation]
**Learning:** Secondary links on landing pages (like API docs, settings, and auth links) often miss standard keyboard focus states if they aren't standard buttons, making the initial page inaccessible to keyboard users.
**Action:** Always verify that every navigation `<Link>` and `<a>` tag on landing and unauthenticated pages includes standard `focus-visible:outline-none focus-visible:ring-2` styles, adding `rounded` constraints to inline text links to ensure the focus ring aligns correctly.
